...
- [x] **[CRITICAL]** Real benchmark execution against standard datasets — `src/evals/dataset_runners.py`: 5 publishable dataset runners (GSM8K, TruthfulQA, HumanEval, MMLU, HellaSwag) with inline reference sample sets (no network required) + optional live HuggingFace loading (`BENCHMARK_USE_HF_DATASETS=true`). Each run produces a `DatasetBenchmarkResult` with SHA-256 content hash for reproducibility attestation. Export pipeline in `src/evals/artifact_export.py` produces JSONL, CSV, HTML report, leaderboard JSON (Papers With Code / OpenLLM schema), and a signed manifest. `src/benchmark.py` exposes `run_dataset_benchmark()`, `run_dataset_suite_benchmark()`, `export_benchmark_run()`. API: `POST /benchmark/dataset/run`, `POST /benchmark/dataset/suite`, `GET /benchmark/dataset/history`, `GET /benchmark/export/{run_id}`, `GET /benchmark/export/{run_id}/html`.
...
- [x] **[CRITICAL]** Benchmark credibility gap — Closed (commit d3a9ad3). `src/evals/dataset_runners.py` adds 5 publishable dataset runners (GSM8K, TruthfulQA, HumanEval, MMLU, HellaSwag) with inline reference samples + optional HuggingFace live loading, reproducibility-attested SHA-256 content hashes, and `DatasetBenchmarkResult` metadata. `src/evals/artifact_export.py` exports to JSONL, CSV, HTML (self-contained), leaderboard JSON (Papers With Code / OpenLLM schema v1.0), and a signed SHA-256 manifest. 7 new API routes under `/benchmark/dataset/` and `/benchmark/export/`. 47 tests.
...
- [x] **[HIGH]** SDK shipping and operator-grade packaging gap — Closed (commit d3a9ad3). Python: `pyproject.toml` (hatchling, classifiers, Python 3.9–3.13 matrix, `[async]` extras), `_version.py` (semver), `AsyncNexusAIClient` (httpx, async context manager, streaming), `NexusOperator` (exponential-backoff retry, env-var config, thread-safe singleton, health verification), `compat.py` (`validate()` → `CompatReport`). TypeScript: `tsconfig.json` (strict TS5, declaration maps), `package.json` (exports/engines/prepublishOnly), `src/index.ts`, `NexusOperator` (retry, dataset benchmark methods). Go: `nexusai/operator.go` (`Operator`, `RetryConfig`, `BenchmarkDataset/Suite/Export`, `IsHealthy`, `CompatibilityReport`, `SDKVersion`). All three SDKs extended with dataset benchmark client methods. 48 SDK packaging tests.

#### Immediate sprint execution grid (P0/P1)

| Workstream | Priority | Owner | Acceptance criteria | Test gates |
|---|---|---|---|---|
| ~~Real benchmark runners and comparable publication~~ | ~~`P0`~~ | — | **DONE** (commit d3a9ad3) — `src/evals/dataset_runners.py` + `src/evals/artifact_export.py` + 7 new API routes. GSM8K, TruthfulQA, HumanEval, MMLU, HellaSwag runners. JSONL/CSV/HTML/leaderboard JSON/manifest export. 47 tests passing. | ✅ `pytest tests/test_benchmark_datasets.py` — 47 passed |
